...
import { singleFanNotification, apiError } from '../../../store/actions';
...
import '../../../assets/css/celebritycss/celebrity.css';
 ...
 const SingleFanModal = (props) => {
    const { modal_loading, error, notify_success, set_modal, fan } = props;
    console.log(fan.user.username)

    const handleValidSubmit = (event, values) => {
        console.log(singleFanNotification);
        event.preventDefault();
        const single_fan_notification = { ...{fan_id: props.match.params.id }, ...values }
        console.log(single_fan_notification);
        props.singleFanNotification(single_fan_notification);
    };
        ...
        <div>
            ...
            >
                <Link to="#" className="text-success text-center font-weight-bolder">
                    ...
                    <p>Sending Celebrity Push Notification to {fan.user.username}</p>
                </Link>
            </div>
            <ModalBody className="modal-contentt"style={{background:'#1c1c1c'}}>
                <div className="col-md-12">
                    <p className="push-celebs mb-2 mt-2"style={{ textAlign: "left", textTransform:'capitalize'}}>Send Push Notification to {fan.user.username}</p>
...
// export default  SingleFanModal;

const mapStateToProps = state => {
    const { error, modal_loading, notify_success, fans } = state.FanPushNotifications;
;
    return { error, modal_loading, notify_success, fans };
};
export default withRouter(connect(mapStateToProps, { singleFanNotification, apiError })(SingleFanModal));
