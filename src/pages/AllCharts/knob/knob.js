...
class Knob extends React.Component {
	...
	constructor(props) {
		...
		this.cursorExt =
			this.props.cursor === true ? 0.3 : this.props.cursor / 100;
	...
	eventToValue = (e) => {
		...
		const val = !this.props.log
			? (a * (this.props.max - this.props.min)) / this.angleArc +
			  this.props.min
	...
	handleTouchStart = (e) => {
		...
		this.props.onChange(
			this.eventToValue(e.targetTouches[this.touchIndex])
		);
	...
	handleTextInput = (e) => {
		const val =
			Math.max(
				Math.min(+e.target.value, this.props.max),
				this.props.min
			) || this.props.min;
	...
	render() {
		...
		return (
			<div
				...
				onWheel={
					readOnly || disableMouseWheel ? null : this.handleWheel
				}
			>
				<canvas
					ref={(ref) => {
